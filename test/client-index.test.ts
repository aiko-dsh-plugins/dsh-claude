...
describe('Claude client slot registration', () => {
  it('registers the transcript node unconditionally so the renderer choice is read per step', () => {
    ...
    expect(captured.registrations.filter(entry => entry.name.startsWith('conversation.input.'))).toMatchObject([{ name: 'conversation.input.dock', id: 'claude-review-comments' }])
    expect(captured.registrations.filter(entry => entry.name === 'conversation.session.header.utilities')).toMatchObject([{ id: 'claude-plan' }, { id: 'claude-diff' }, { id: 'claude-tasks' }])
